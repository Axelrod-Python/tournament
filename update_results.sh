...
python3 run_axelrod --noise 0.05 -p $processes -o assets -r $repetitions --xc
...
python3 run_axelrod -p $processes -o assets -r $repetitions --xc

# Running prob end tournament with p_end=0.1
echo "-- Running Prob End Axelrod, regenerating cache, results, and figures"
# Also Copy the results and figures over to assets
python3 run_axelrod -pe 0.1 -p $processes -o assets -r $repetitions --xc
